"""
╔══════════════════════════════════════════════════════════════════════╗
║         FOREXIA AI AGENT — GEMINI MARKET INTELLIGENCE & EXECUTION   ║
║   AI-Powered Market Analysis + Autonomous Trade Execution            ║
╚══════════════════════════════════════════════════════════════════════╝
...
This engine connects to Google Gemini to provide:
  ...
  • AUTONOMOUS TRADE EXECUTION — AI generates and executes trade signals
...
class GeminiAdvisor:
    ...
    async def _call_gemini(self, prompt: str, system: str = SYSTEM_PROMPT) -> Optional[str]:
        ...
        for model in models_to_try:
            ...
            # Build payload — basic models don't support systemInstruction or JSON mode
            if is_basic:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(f"Gemini ({model}) rate limited (429) — blocked for {retry_delay}s, trying next model...")
                    continue
                ...
                logger.warning(f"Gemini ({model}) API timeout — trying next model...")
                continue
        ...
        logger.warning("All Gemini/Gemma models exhausted — no AI response available")
        return None
    ...
    async def generate_trade_signal(
        ...
        if action not in ("BUY", "SELL"):
            logger.info(f"[GEMINI] {symbol} — AI decided to PASS (no trade)")
            return None
        ...
        logger.info(
            f"[GEMINI] 🔥 AI TRADE SIGNAL: {action} {symbol} @ {signal.entry_price:.5f} | "
